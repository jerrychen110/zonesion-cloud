...
(function(angular){
    ...
    .constant('ngJcropTemplate',
        ...
        '   </div>' +
        '   <span class="jcrop-label" ng-show="jcropLabel" translate="{{jcropLabel}}" style="width:{{previewImgStyle.width}}">Image Preview</span>' +
        '</div>'
    )
    ...
    .directive('ngJcrop', ['ngJcropConfig', function(ngJcropConfig){
        ...
        return {
            restrict: 'A',
            scope: { ngJcrop: '=', thumbnail: '=', selection: '=', ngJcropConfigName: '@', jcropLabel:'@' },
    ...
    function($scope, $element, ngJcropConfig){
        ...
        $scope.updateCurrentSizes = function(img){
            ...
            if (widthConstraining) {
                $scope.imgStyle.width = Math.floor(ngJcropConfig.jcrop.maxWidth);
                $scope.imgStyle.height = Math.floor(img.height / widthShrinkRatio);
            } else if (heightConstraining) {
                $scope.imgStyle.height = Math.floor(ngJcropConfig.jcrop.maxHeight);
                $scope.imgStyle.width = Math.floor(img.width / heightShrinkRatio);
            } else if(squareConstraining){
                $scope.imgStyle.height = Math.floor(ngJcropConfig.jcrop.maxHeight);
                $scope.imgStyle.width = Math.floor(ngJcropConfig.jcrop.maxWidth);
            } else {
                $scope.imgStyle.height = Math.floor(img.height);
                $scope.imgStyle.width = Math.floor(img.width);
            }
        };
        ...
        $scope.setSelection = function(coords){
            ...
            $scope.selection[0] = Math.floor(coords.x * shrinkRatio);
            $scope.selection[1] = Math.floor(coords.y * shrinkRatio);
            $scope.selection[2] = Math.floor(coords.x2 * shrinkRatio);
            $scope.selection[3] = Math.floor(coords.y2 * shrinkRatio);
            $scope.selection[4] = Math.floor(coords.w * shrinkRatio);
            $scope.selection[5] = Math.floor(coords.h * shrinkRatio);
            if(img.width<$scope.selection[2]){
              $scope.selection[2] = img.width;
              $scope.selection[4] = $scope.selection[2]-$scope.selection[0];
            }
            if(img.height<$scope.selection[3]){
              $scope.selection[3] = img.height;
              $scope.selection[5] = $scope.selection[3]-$scope.selection[1];
            }
            if($scope.selection[4]>$scope.selection[5]){
              $scope.selection[4] = $scope.selection[5];
            }else {
              $scope.selection[5] = $scope.selection[4];
            }
        };
