...
import com.zonesion.cloud.service.UserInfoService;
...
public class UserInfoResource {
    ...
    @GetMapping("/user/{id}")
    @Timed
    public ResponseEntity<List<UserInfoDTO>> getUserLearned(@PathVariable Long id) {
        log.debug("REST request to get a page of users");
        List<UserInfoDTO> userInfo = userInfoService.findUserLearned(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(userInfo));
    }
    
    @GetMapping("/user/{id}/favorited")
    @Timed
    public ResponseEntity<List<UserInfoDTO>> getUserFavorited(@PathVariable Long id) {
        log.debug("REST request to get a page of users");
        List<UserInfoDTO> userInfo = userInfoService.findUserFavorited(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(userInfo));
    }
