...
public interface CourseRepository extends PagingAndSortingRepository<Course, Long>, JpaSpecificationExecutor<Course> {
	...
	//查询最新课程
	@Query(value="SELECT * FROM t_course ORDER BY last_modified_date DESC \n#pageable\n",
			countQuery = "SELECT count(*) FROM t_course",nativeQuery = true)
	Page<Course> findNewestCourseByUserId(Pageable pageable);
	
	//查询最热课程
	@Query(value="SELECT c.*, cl.* FROM t_course c LEFT JOIN t_chapter ch ON c.id = ch.course_id LEFT JOIN t_course_lesson cl ON cl.chapter_id = ch.id WHERE c.recommended='是' AND cl.learned_num > 0 ORDER BY cl.learned_num DESC \n#pageable\n",
			countQuery = "SELECT count(*) FROM t_course",nativeQuery = true)
	Page<Course> findHotCourseByUserId(Pageable pageable);
	
	
	//查询推荐课程
	@Query(value="SELECT c.* FROM t_course c WHERE c.recommended='是' ORDER BY recommended_sort DESC \n#pageable\n",
			countQuery = "SELECT count(*) FROM t_course",nativeQuery = true)
